...
/// Historique des effectifs de départ écrasés par une resaisie, pour une ferme + un lot
/// donnés — page séparée (accessible via un simple bouton "Voir l'historique" depuis l'écran
/// de saisie) et paginée (skip/limit + total, même pattern que les notifications côté
/// routers/notifications.py) car cet historique n'a pas de limite pratique dans le temps.
class LotHeadcountHistoryScreen extends StatefulWidget {
  final String farmName;
  final String lotNumber;

  const LotHeadcountHistoryScreen({super.key, required this.farmName, required this.lotNumber});

  @override
  State<LotHeadcountHistoryScreen> createState() => _LotHeadcountHistoryScreenState();
}

class _LotHeadcountHistoryScreenState extends State<LotHeadcountHistoryScreen> {
  final MongoService _mongoService = MongoService();
  List<LotHeadcountHistoryEntry> _entries = [];
  ...
  Future<void> _load() async {
    ...
    final result = await _mongoService.getLotHeadcountHistory(widget.farmName, widget.lotNumber, skip: 0, limit: _pageSize);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _loadMore() async {
    ...
    final result = await _mongoService.getLotHeadcountHistory(
      widget.farmName,
      widget.lotNumber,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: dailyReportAppBar(widget.farmName, subtitle: 'Historique — Lot ${widget.lotNumber}'),
      body: _isLoading
  ...
  Widget _entryCard(LotHeadcountHistoryEntry h) {
    ...
    return DailyReportCard(
      ...
      children: [
        ...
        for (final r in h.rooms)
          if (r.femaleCount > 0 || r.maleCount > 0) _row(r.roomName, r.femaleCount, r.maleCount),
        if (h.clinicFemaleCount > 0 || h.clinicMaleCount > 0) _row('Clinique', h.clinicFemaleCount, h.clinicMaleCount),
        const Divider(height: 16),
        Row(
          children: [
            const Expanded(child: Text('Total', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13))),
            Text(
              '${h.totalFemale} F · ${h.totalMale} M',
  ...
  Widget _row(String label, int female, int male) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 2),
      child: Row(
        children: [
          Expanded(child: Text(label, style: const TextStyle(fontSize: 12.5))),
          Text('$female F · $male M', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 12.5)),
        ],
      ),
    );
  }
